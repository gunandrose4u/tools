...
class Benchmarker(ABC):
    def __init__(self, config, data_loader, backend, batch_size, metrics_analysis):
...
class DirectBenchmarker(Benchmarker):
    def __init__(self, config, data_loader, backend, batch_size, metrics_analysis=None):
        super(DirectBenchmarker, self).__init__(config, data_loader, backend, batch_size, metrics_analysis)
    ...
    def run(self):
        ...
        if left_sample_cnt > 0:
            self._test_times += 1
            self._backend.predict_with_perf(self._data_loader.get_batch_items(left_sample_cnt))
        t2_end = perf_counter()
        ...
        res_benchmark['qps'] = self._i_total_samples / np.sum(self._backend.predict_times)
